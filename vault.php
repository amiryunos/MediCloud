...
// Handle file uploads and directory creation
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['fileToEncrypt'])) {
    $vaultName = trim($_POST['vaultname']); // Get the vault name from the user input
    if (empty($vaultName)) {
        echo 'Vault name cannot be empty.';
        exit;
    }
    
    $vaultDir = 'vaults/' . preg_replace('/[^a-zA-Z0-9_-]/', '', $vaultName); // Create a safe directory name
    if (!is_dir($vaultDir)) {
        mkdir($vaultDir, 0777, true); // Create the directory if it doesn't exist
    }
    
    foreach ($_FILES['fileToEncrypt']['tmp_name'] as $key => $tmp_name) {
        $fileName = $_FILES['fileToEncrypt']['name'][$key];
        $filePath = $vaultDir . '/' . $fileName;
        move_uploaded_file($tmp_name, $filePath); // Move the uploaded file to the directory
        // Encrypt the file
        $encryptedFilePath = $filePath . '.enc';
        encryptFile($filePath, $encryptedFilePath, $newAESKey);
        unlink($filePath); // Remove the original file
    }
    
    echo 'Files have been encrypted and stored in the vault.';
}
...
//     $encryptedData = openssl_encrypt($data, $cipher, hex2bin($key), 0, $iv);
...
<body>
    ...
    <form id="encryptForm" method="post" action="encrypt.php" enctype="multipart/form-data" onsubmit="onSubmit(event)">
        ...
        <label for="vault name">Enter vault Name:</label>
        <input type="name" id="vaultName" name="vaultName" required placeholder="enter vault name">

        <label for="fileToEncrypt">Choose File:</label>
        <input type="file" id="fileToEncrypt" name="fileToEncrypt[]" multiple required>
    ...
    <script src="scipt_vault.js"></script>
    <script>
        function refreshKey() {
    ...
    // Jika tidak ada fail, keluar dari fungsi
    if (!fileInput.files.length) return;

    // Buat senarai fail sedia ada
    const existingFiles = Array.from(fileListDiv.getElementsByClassName('file-list-item')).map(item => item.textContent);

    // Tambah fail yang dipilih
    Array.from(fileInput.files).forEach((file) => {
        // Jika fail sudah ada dalam senarai, jangan tambah lagi
